# 백테스트 빠른 시작 가이드

이동평균 교차 전략을 과거 데이터로 백테스트하는 방법입니다.

---

## 🚀 빠른 시작

### 1️⃣ FinanceDataReader로 백테스트 (권장)

무료, 무제한 데이터로 장기 백테스트:

```bash
# trading_bot 폴더에서 실행 (권장)
cd trading_bot
uv run run_backtest.py --source fdr --start 20220101 --end 20241231

# 1년 백테스트
uv run run_backtest.py --source fdr --start 20230101

# 특정 종목만
uv run run_backtest.py --source fdr --symbols 005930 000660 035720
```

**설치 필요:**
```bash
uv pip install finance-datareader
```

### 2️⃣ KIS API로 백테스트

최근 100거래일 데이터:

```bash
# trading_bot 폴더에서
cd trading_bot
uv run run_backtest.py --source api

# 특정 기간 (최대 100일)
uv run run_backtest.py --source api --start 20240901 --end 20241231
```

**설정 필요:** `kis_devlp.yaml`에 API 키 등록

### 3️⃣ 외부 DB로 백테스트

직접 준비한 SQLite DB 사용:

```bash
# trading_bot 폴더에서
cd trading_bot
uv run run_backtest.py --source db --db-path backtest_data.db --start 20200101
```

---

## 📊 주요 옵션

### 데이터 소스 선택

```bash
--source {api|db|fdr}
```

- `api`: KIS Open Trading API (최대 100일)
- `db`: 외부 SQLite DB (무제한)
- `fdr`: FinanceDataReader (무제한, 권장)

### 기간 설정

```bash
--start YYYYMMDD    # 시작일
--end YYYYMMDD      # 종료일
```

생략 시 기본값:
- `api`: 최근 100일
- `db`, `fdr`: 최근 2년

### 종목 선택

```bash
--symbols 005930 000660 035720
```

생략 시 `Config.WATCH_LIST` 사용

### 전략 파라미터

```bash
--short-period 5    # 단기 이동평균 (기본값: 5)
--long-period 20    # 장기 이동평균 (기본값: 20)
```

### 초기 자본금

```bash
--capital 10000000  # 기본값: 1천만원
```

### 결과 저장

```bash
--output backtest_results/custom_name.png  # 파일명 지정
```

생략 시: `backtest_results/{전략이름}_{데이터소스}_{타입}_{날짜}_{시간}.{확장자}`

**예시:**
- FDR: `backtest_results/ma_crossover_fdr_chart_20240106_143000.png`
- API: `backtest_results/ma_crossover_api_chart_20240106_143000.png`
- DB: `backtest_results/ma_crossover_db_chart_20240106_143000.png`

---

## 📖 예시 명령어

**💡 모든 명령어는 `trading_bot/` 폴더에서 실행합니다:**

```bash
cd trading_bot
```

### 1. 기본 백테스트 (FDR, 2년)

```bash
uv run run_backtest.py
```

### 2. 삼성전자만 5년 백테스트

```bash
uv run run_backtest.py --source fdr --symbols 005930 --start 20190101 --end 20241231
```

### 3. 전략 파라미터 조정

```bash
# 단기 3일, 장기 10일 이동평균
uv run run_backtest.py --source fdr --short-period 3 --long-period 10
```

### 4. 외부 DB 사용

```bash
# 먼저 DB 생성
uv run create_external_db.py

# 백테스트 실행
uv run run_backtest.py --source db --db-path backtest_data.db
```

### 5. 최근 100일 빠른 테스트

```bash
uv run run_backtest.py --source api
```

---

## 📈 결과 해석

백테스트 완료 후 `backtest_results/` 폴더에 다음 파일이 생성됩니다:

1. **차트 이미지** (PNG): `{전략이름}_{데이터소스}_chart_{날짜}_{시간}.png`
   - 자산 곡선, 보유 종목 수, 누적 수익률 그래프
2. **자산 곡선** (CSV): `{전략이름}_{데이터소스}_equity_{날짜}_{시간}.csv`
   - 일별 자산 가치 데이터
3. **거래 내역** (CSV): `{전략이름}_{데이터소스}_trades_{날짜}_{시간}.csv`
   - 모든 매수/매도 거래 상세 내역

**파일 예시:**
```
backtest_results/
├── ma_crossover_fdr_chart_20260106_144425.png
├── ma_crossover_fdr_equity_20260106_144425.csv
├── ma_crossover_fdr_trades_20260106_144425.csv
├── ma_crossover_api_chart_20260106_145030.png
└── ma_crossover_db_chart_20260106_150000.png
```

**데이터 소스별 비교:**
- `fdr`: FinanceDataReader (장기 백테스트)
- `api`: KIS API (최근 100일)
- `db`: 외부 SQLite DB (커스텀 데이터)

### 결과 예시

```
======================================================================
                              백테스트 결과 요약                       
======================================================================

📅 백테스트 기간
  시작일: 2022-01-03
  종료일: 2024-12-30

💰 자본금 및 수익
  초기 자본:         10,000,000원
  최종 자본:          9,742,253원
  총 수익:             -257,747원
  총 수익률:              -2.58%
  연평균 수익률:          -0.87%

📊 리스크 지표
  최대 낙폭(MDD):          4.12%
  샤프 비율:              -1.05

📈 거래 통계
  총 거래 횟수:              85회
  승률:                   35.29%
  손익비:                  0.78
  평균 손익:             -3,032원
```

---

## 🔧 문제 해결

### FinanceDataReader 설치 오류

```bash
❌ FinanceDataReader가 설치되지 않았습니다!
```

**해결:**
```bash
uv pip install finance-datareader
```

### DB 파일 없음

```bash
❌ 오류: DB 파일이 없습니다: /path/to/data.db
```

**해결:**
1. 경로 확인: `--db-path` 옵션 확인
2. DB 생성: `uv run create_external_db.py`
3. 또는 FDR 사용: `--source fdr`

### KIS API 인증 실패

```bash
❌ 인증 실패: Invalid API key
```

**해결:**
1. `kis_devlp.yaml` 파일 확인
2. API 키 재발급
3. 또는 FDR 사용: `--source fdr`

### 거래 내역 없음

```bash
⚠️  경고: 거래 내역이 없습니다.
```

**원인:**
- 기간이 너무 짧음
- 전략 조건 미충족

**해결:**
1. 기간 늘리기: `--start 20200101`
2. 전략 파라미터 조정: `--short-period 3 --long-period 10`
3. 종목 변경: `--symbols 005930 000660`

---

## 📚 자세한 문서

- [데이터 소스 비교](DATA_SOURCES.md) - 세 가지 데이터 소스 상세 비교
- [외부 DB 생성](DB_GUIDE.md) - SQLite DB 만들기
- [전략 개발 가이드](../README.md) - 새 전략 추가 방법

---

## 💡 팁

### 빠른 검증

단기간 백테스트로 전략 빠르게 검증:

```bash
# 최근 6개월
uv run run_backtest.py --source fdr --start 20240601
```

### 파라미터 최적화

다양한 이동평균 기간 테스트:

```bash
# 3/10
uv run run_backtest.py --short-period 3 --long-period 10

# 5/20 (기본값)
uv run run_backtest.py --short-period 5 --long-period 20

# 10/50
uv run run_backtest.py --short-period 10 --long-period 50
```

### 다중 종목 비교

개별 종목 vs 포트폴리오:

```bash
# 삼성전자만
uv run run_backtest.py --symbols 005930

# 포트폴리오
uv run run_backtest.py --symbols 005930 000660 035720 035420
```

---

## ⚡ 성능 비교

| 데이터 소스 | 속도 | 기간 제한 | 인터넷 | 추천 |
|------------|------|----------|--------|------|
| **api** | 느림 | 100일 | 필요 | ❌ |
| **db** | 빠름 | 없음 | 불필요 | ⭐ (오프라인) |
| **fdr** | 보통 | 없음 | 필요 | ⭐⭐ (권장) |

**권장 사용법:**
- 개발/테스트: `fdr` (간편)
- 프로덕션: `db` (빠름)
- 단기 검증: `api` (설치 불필요)

# 백테스트 가이드

## 📊 백테스트란?

백테스트(Backtest)는 전략을 과거 데이터에 적용하여 성과를 검증하는 과정입니다.

### 왜 백테스트가 필요한가?
- ✅ 전략의 수익성 검증
- ✅ 리스크 파악 (최대 낙폭 등)
- ✅ 파라미터 최적화
- ✅ 실전 투자 전 신뢰도 확보

## 🚀 빠른 시작

> 💡 **더 자세한 내용은 [backtest/QUICKSTART.md](backtest/QUICKSTART.md)를 참고하세요.**

### 1. 백테스트 실행

**3가지 데이터 소스 중 선택:**

```bash
# trading_bot 폴더에서 실행
cd trading_bot

# 방법 1: FinanceDataReader (권장 - 무료, 무제한)
uv run run_backtest.py --source fdr --start 20220101 --end 20241231

# 방법 2: KIS API (최근 100거래일)
uv run run_backtest.py --source api

# 방법 3: 외부 DB (무제한, 오프라인 가능)
uv run run_backtest.py --source db --db-path backtest_data.db --start 20200101
```

### 2. 주요 옵션

```bash
--source {api|db|fdr}    # 데이터 소스 선택 (기본값: fdr)
--start YYYYMMDD         # 시작일 (기본값: 2년 전)
--end YYYYMMDD           # 종료일 (기본값: 어제)
--symbols 005930 000660  # 종목 지정 (기본값: Config.WATCH_LIST)
--capital 10000000       # 초기 자본금 (기본값: 1천만원)
--short-period 5         # 단기 이동평균 (기본값: 5)
--long-period 20         # 장기 이동평균 (기본값: 20)
```

### 환경 변수 및 .env 파일

- `trading_bot/config.py`는 시스템 환경변수와 프로젝트 루트의 `.env` 파일을 읽어 일부 설정을 초기화합니다. 우선순위는 시스템 환경변수 > `.env` > 코드 기본값입니다.
- 백테스트는 원칙적으로 실거래 주문을 발생시키지 않지만, 환경 설정(`ENV_MODE`, `TRADING_ENABLED`)은 `trading_bot/config.py`에서 불러오므로 로컬 실행 시 의도한 동작이 되도록 `.env` 또는 환경변수를 확인하세요.
- 권장: 백테스트 실행 시 `TRADING_ENABLED`는 `false`로 설정하거나, 백테스트 전용 실행 스크립트를 사용하세요.

예시 `.env` (프로젝트 루트):

```
ENV_MODE=demo
TRADING_ENABLED=false
```

`.env.sample` 파일이 저장소 루트에 존재하므로 이를 복사해 `.env`로 사용하세요.


### 3. 결과 확인

실행 후 다음 정보가 출력됩니다:
- 📅 백테스트 기간
- 💰 초기/최종 자본, 수익률
- 📊 MDD, 샤프비율
- 📈 거래 통계 (승률, 손익비 등)

### 4. 결과 파일

`backtest_results/` 디렉토리에 저장:

```
backtest_results/
├── ma_crossover_fdr_chart_20260106_144425.png    # 그래프
├── ma_crossover_fdr_equity_20260106_144425.csv   # 자산 곡선
└── ma_crossover_fdr_trades_20260106_144425.csv   # 거래 내역
```

파일명 규칙: `{전략명}_{데이터소스}_{타입}_{날짜}_{시간}.{확장자}`

## 📚 데이터 소스 비교

> 💡 **상세 비교는 [backtest/DATA_SOURCES.md](backtest/DATA_SOURCES.md)를 참고하세요.**

| 데이터 소스 | 기간 제한 | 설치 필요 | 인터넷 | 장점 |
|------------|----------|----------|--------|------|
| **api** | 최근 100일 | ❌ | ✅ | KIS API 테스트 겸용 |
| **db** | 무제한 | DB 생성 | ❌ | 오프라인 가능, 빠름 |
| **fdr** | 무제한 | `uv pip install finance-datareader` | ✅ | 한국 주식 최적화, 무료 |

**권장 순서:**
1. FDR로 빠른 검증
2. 장기 백테스트 필요 시 외부 DB 생성
3. KIS API는 100일 제한으로 간단 테스트용

## ⚙️ 설정 변경

### 명령행 옵션으로 변경 (권장)

```bash
# 종목 변경
uv run run_backtest.py --symbols 005930 000660 035720

# 전략 파라미터 변경
uv run run_backtest.py --short-period 3 --long-period 10

# 초기 자본금 변경
uv run run_backtest.py --capital 20000000

# 종합 예시
uv run run_backtest.py --source fdr --start 20200101 --symbols 005930 --capital 50000000
```

### config.py에서 기본값 설정

```python
# 백테스트 설정
BACKTEST_INITIAL_CAPITAL = 10000000  # 초기 자본금
BACKTEST_COMMISSION_RATE = 0.00015   # 수수료율 (0.015%)
BACKTEST_SLIPPAGE_RATE = 0.001       # 슬리피지 (0.1%)

# 기본 감시 종목
WATCH_LIST = [
    "005930",  # 삼성전자
    "000660",  # SK하이닉스
]

# 전략 파라미터 (명령행에서 override 가능)
MA_SHORT_PERIOD = 5   # 단기 이동평균
MA_LONG_PERIOD = 20   # 장기 이동평균
```

## 📈 성과 지표 설명

### 수익률 지표
- **총 수익률**: (최종자본 - 초기자본) / 초기자본 × 100
- **CAGR**: 연평균 복리 수익률
- **평균 손익**: 거래당 평균 손익

### 리스크 지표
- **MDD (Maximum Drawdown)**: 최대 낙폭
  - 고점 대비 최대 하락폭
  - 낮을수록 안정적
- **샤프 비율**: 위험 대비 수익률
  - 높을수록 효율적
  - 1.0 이상이면 양호, 2.0 이상이면 우수

### 거래 통계
- **승률**: 수익 거래 비율
- **손익비**: 총 이익 / 총 손실
  - 1.0 이상이면 양호
- **총 거래 횟수**: 매매 빈도

## 💡 사용 예시

### 예시 1: 다른 종목으로 테스트

```bash
# 삼성전자만
uv run run_backtest.py --symbols 005930

# 여러 종목
uv run run_backtest.py --symbols 005380 051910 035720
```

### 예시 2: 파라미터 최적화

```bash
# 단기 3일, 장기 10일
uv run run_backtest.py --short-period 3 --long-period 10

# 단기 5일, 장기 20일 (기본값)
uv run run_backtest.py --short-period 5 --long-period 20

# 단기 10일, 장기 50일
uv run run_backtest.py --short-period 10 --long-period 50
```

결과를 비교하여 최적 파라미터 선택

### 예시 3: 긴 기간 테스트

```bash
# FDR로 5년 백테스트
uv run run_backtest.py --source fdr --start 20190101 --end 20241231

# 외부 DB로 10년 백테스트
uv run create_external_db.py  # DB 먼저 생성
uv run run_backtest.py --source db --db-path backtest_data.db --start 20140101
```

### 예시 4: 외부 DB 생성 및 사용

> 💡 **상세 가이드는 [backtest/DB_GUIDE.md](backtest/DB_GUIDE.md)를 참고하세요.**

```bash
# 1. DB 생성
uv run create_external_db.py

# 2. DB로 백테스트
uv run run_backtest.py --source db --db-path backtest_data.db --start 20200101
```

## 📊 결과 분석 방법

### 1. 자산 곡선 확인
- 지속적 상승: 안정적 전략
- 변동성 큼: 리스크 높음
- 하락 지속: 전략 개선 필요

### 2. MDD 확인
- 10% 미만: 양호
- 10-20%: 보통
- 20% 이상: 주의 (손절 전략 필요)

### 3. 승률과 손익비 함께 보기
- 승률 높고 손익비 낮음: 소액 다수 매매
- 승률 낮고 손익비 높음: 손절 철저, 큰 수익
- 둘 다 높음: 이상적

### 4. 거래 횟수 확인
- 너무 많음: 수수료 부담, 과최적화 가능성
- 너무 적음: 우연일 수 있음, 더 많은 데이터 필요

## ⚠️ 주의사항

### 백테스트의 한계
1. **과거 데이터 기반**: 미래를 보장하지 않음
2. **과최적화 위험**: 과거에만 맞는 파라미터일 수 있음
3. **실제와 차이**: 
   - 슬리피지 (실제는 더 클 수 있음)
   - 유동성 (대량 주문 시 가격 영향)
   - 심리적 요인 (실전에서의 판단 오류)

### 백테스트 후 할 일
1. ✅ **여러 기간으로 테스트**: 다양한 시장 상황 검증
2. ✅ **여러 종목으로 테스트**: 일반화 가능성 확인
3. ✅ **모의투자 실행**: 실시간 데이터로 재검증
4. ✅ **소액 실전 투자**: 실제 시장에서 최종 검증

### 좋은 백테스트 전략
- MDD < 15%
- 샤프 비율 > 1.0
- 승률 > 50% 또는 손익비 > 2.0
- 거래 횟수 > 30회 (통계적 유의성)
- 여러 기간에서 일관된 성과

## 🔧 고급 사용법

### 1. 커스텀 백테스트 스크립트

```python
from trading_bot.backtest.engine import BacktestEngine
from trading_bot.backtest.report import BacktestReport
from trading_bot.strategies.ma_crossover import MovingAverageCrossover
from trading_bot.broker.kis_broker import KISBroker

# 설정
broker = KISBroker(env_mode="demo")
strategy = MovingAverageCrossover(broker, short_period=5, long_period=20)
engine = BacktestEngine(initial_capital=10000000)

# 실행 (3가지 방법)

# 방법 1: FDR 사용
results = engine.run(
    strategy=strategy,
    symbols=["005930"],
    start_date="20230101",
    end_date="20231231",
    use_fdr=True
)

# 방법 2: KIS API 사용
results = engine.run(
    strategy=strategy,
    symbols=["005930"],
    start_date="20230101",
    end_date="20231231",
    broker=broker
)

# 방법 3: 외부 DB 사용
from pathlib import Path
results = engine.run(
    strategy=strategy,
    symbols=["005930"],
    start_date="20230101",
    end_date="20231231",
    db_path=Path("backtest_data.db")
)

# 결과 확인
print(results['metrics'])
BacktestReport.print_summary(results)
```

### 2. 새로운 전략 백테스트

1. `trading_bot/strategies/`에 새 전략 작성
2. `run_backtest.py`에서 전략 변경:
   ```python
   from trading_bot.strategies.my_strategy import MyStrategy
   strategy = MyStrategy(broker, param1=value1, param2=value2)
   ```

### 3. 여러 종목 동시 백테스트

```bash
# 모든 Config.WATCH_LIST 종목
uv run run_backtest.py --source fdr --start 20220101

# 특정 여러 종목
uv run run_backtest.py --source fdr --symbols 005930 000660 035720 051910
```

## 📚 다음 단계

백테스트 완료 후:
1. 📝 결과 분석 및 파라미터 조정
2. 🔄 다양한 조건에서 재테스트 (여러 기간, 여러 종목)
3. 🎮 모의투자로 실시간 검증 (`ENV_MODE = "demo"`)
4. 💰 소액 실전 투자
5. 📈 성과 모니터링 및 전략 개선

## 📖 관련 문서

- **[backtest/QUICKSTART.md](backtest/QUICKSTART.md)** - 백테스트 빠른 시작
- **[backtest/DATA_SOURCES.md](backtest/DATA_SOURCES.md)** - 데이터 소스 비교 및 선택
- **[backtest/DB_GUIDE.md](backtest/DB_GUIDE.md)** - 외부 DB 생성 가이드
- **[README.md](README.md)** - 자동매매 봇 전체 개요

---

**중요**: 백테스트는 시작일 뿐입니다. 반드시 모의투자와 소액 실전으로 검증하세요!

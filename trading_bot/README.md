...
- **KRX 기준 호가 단위 규칙 적용**
  ...
  - **100,000 ~ 500,000원: 500원** ← 셀트리온 사례
  - 500,000원 이상: 1,000원
- **자동 검증 및 조정**:
  - 주문 전 호가 단위 자동 검증 (`_validate_price_tick_unit`)
  - 위반 시 자동 조정 (`_adjust_price_to_tick_unit`)
  - 모의투자/실전투자 모두 동일 규칙 적용
- **상세 가이드**: [TICK_UNIT_GUIDE.md](./TICK_UNIT_GUIDE.md)
...
`trading_bot/config.py`에 전략별 설정을 한 곳에서 관리하려면 `STRATEGIES_CONFIG_TEMPLATE`을 사용할 수 있습니다.
이 템플릿은 전략 이름과 해당 전략에 전달할 `config` 딕터를 포함하는 리스트입니다.
